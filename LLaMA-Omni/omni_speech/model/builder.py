...
def create_model_lora(model_path, model_base, is_lora=False, merge_lora=False, s2s=False, load_8bit=False, load_4bit=False, device="cuda", use_flash_attn=False, freeze_speech_encoder=True,        **kwargs):
    ...
    # Tokenizer
    tokenizer = AutoTokenizer.from_pretrained(model_base if is_lora else model_path, use_fast=False)

    if is_lora:
        print("Loading base model for LoRA training...")
        model = model_cls.from_pretrained(model_base, low_cpu_mem_usage=False, **kwargs)
        ...
        lora_config = LoraConfig(
            ...
            lora_alpha=32,
            target_modules=[
                "q_proj", "k_proj", "v_proj", "o_proj", "gate_proj", "up_proj", "down_proj", "speech_projector.linear1", "speech_projector.linear2"
            ],
            lora_dropout=0.1,
            bias="none",
            task_type="CAUSAL_LM"
    ...
    # Speech encoder
    if s2s and hasattr(model, "initialize_speech_generator"):
        print("Initializing speech generator module for s2s model...")
        model.initialize_speech_generator(model.config)
    model = model.to(device=device, dtype=torch.bfloat16)
    ...
    if freeze_speech_encoder:
        print("Freezing speech encoder weights.")
        for param in model.get_model().speech_encoder.parameters():
            param.requires_grad = False
